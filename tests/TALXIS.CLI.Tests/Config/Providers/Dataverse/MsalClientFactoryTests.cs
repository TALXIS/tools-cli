...
public sealed class MsalClientFactoryTests
{
    [Fact]
    public void BuildPublicClient_UsesPacClientIdAndLocalhostRedirect()
    {
        ...
        Assert.Equal(MsalClientFactory.PublicClientId, client.AppConfig.ClientId);
        Assert.Equal("https://login.microsoftonline.com/organizations/", client.Authority);
    }

    [Fact]
    public void BuildPublicClient_UsesSovereignCloud_WhenExplicitlySet()
    {
        var factory = new MsalClientFactory();
        var connection = new Connection
        {
