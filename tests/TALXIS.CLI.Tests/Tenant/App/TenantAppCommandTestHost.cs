...
internal sealed class TenantAppCommandTestHost : IDisposable
{
    private readonly ServiceProvider _provider;

    public TenantAppCommandTestHost(Queue<Func<HttpRequestMessage, HttpResponseMessage>> handlers)
    {
        var services = new ServiceCollection();
        services.AddLogging();
        services.AddSingleton<IConfigurationResolver>(new FixedResolver(TestContext()));
        services.AddSingleton(CreateGraphClient(handlers));
