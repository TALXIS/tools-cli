...
using TALXIS.CLI.Core.Component.Browse;
using TALXIS.CLI.Core.Contracts.Dataverse;
using TALXIS.CLI.Core.DependencyInjection;
using TALXIS.Platform.Metadata;

namespace TALXIS.CLI.Core.Component.Url;
...
public static class UrlBuilder
{
        ...
        ILogger logger)
    {
        ...
        return url is null ? null : new UrlBuilderResult(url, def?.Name ?? typeCode.Value.ToString());
    }
    ...
    private static Uri? BuildAppModuleUrl(string orgUrl, IReadOnlyDictionary<string, string> parameters, ILogger logger)
    {
        ...
        void AddIf(string paramKey, string qsKey)
        {
            var value = Get(parameters, paramKey);
            if (!string.IsNullOrWhiteSpace(value))
                queryParams[qsKey] = value;
        }

        AddIf("entity", "etn");
        AddIf("record", "id");
        AddIf("formid", "formid");
        if (!string.IsNullOrWhiteSpace(Get(parameters, "viewid")))
        {
            queryParams["viewid"] = Get(parameters, "viewid")!;
        ...
        ILogger logger)
    {
        ...
        var solutionId = await ResolveSolutionIdAsync(Get(parameters, "solution"), profileName).ConfigureAwait(false);
        var run = Get(parameters, "run");
        if (!string.IsNullOrWhiteSpace(run))
            return PowerAutomateUrls.FlowRun(environmentId, flowId, run, solutionId);

        var flowView = Get(parameters, "flow-view");
        ...
        ILogger logger)
    {
        ...
        var hideNavbar = string.Equals(Get(parameters, "hidenavbar"), "true", StringComparison.OrdinalIgnoreCase);

        var reserved = new HashSet<string>(StringComparer.OrdinalIgnoreCase) { "id", "screen", "hidenavbar" };
        ...
        return CanvasAppUrls.Play(
            environmentId,
            appId,
            tenantId,
            screen,
            customParams.Count > 0 ? customParams : null,
            hideNavbar);
    }

    private static Uri? BuildReportUrl(string orgUrl, IReadOnlyDictionary<string, string> parameters, ILogger logger)
    {
        var id = Get(parameters, "id");
        ...
        var action = Get(parameters, "report-action") ?? "run";
        ...
        ILogger logger)
    {
        var id = Get(parameters, "id");
        var name = Get(parameters, "name");
        var entity = Get(parameters, "entity");
    ...
    private static Uri? BuildEditorUrl(
        ComponentType typeCode,
        Guid environmentId,
        string? orgUrl,
        Guid componentId,
        ...
        Guid? solutionId)
    {
        ...
        {
            ComponentType.Solution => MakerPortalUrls.Solution(environmentId, componentId),
            ComponentType.Entity => MakerPortalUrls.Entity(environmentId, componentId, solutionId),
            ComponentType.SystemForm when entity != null => MakerPortalUrls.FormDesigner(environmentId, entity, componentId, solutionId),
            ComponentType.Form when entity != null => MakerPortalUrls.FormDesigner(environmentId, entity, componentId, solutionId),
            ComponentType.SavedQuery when entity != null => MakerPortalUrls.ViewDesigner(environmentId, entity, componentId, solutionId),
            ComponentType.Bot => CopilotStudioUrls.BotEditor(environmentId, componentId, solutionId),
            ComponentType.Dataflow => MakerPortalUrls.DataflowEditor(environmentId, componentId),
            ComponentType.Role => MakerPortalUrls.SecurityRoleEditor(environmentId, componentId, solutionId),
    ...
    private static async Task<Guid?> ResolveNameToGuidAsync(ComponentType typeCode, string name, string? profileName, ILogger logger)
    {
        switch (typeCode)
        {
            case ComponentType.Solution:
                var solutionService = TxcServices.Get<ISolutionDetailService>();
                var (solution, _) = await solutionService.ShowAsync(profileName, name, CancellationToken.None).ConfigureAwait(false);
                return solution.Id;
    ...
    private static async Task<Guid?> ResolveSolutionIdAsync(string? solutionUniqueName, string? profileName)
    {
        ...
        var solutionService = TxcServices.Get<ISolutionDetailService>();
        var (solution, _) = await solutionService.ShowAsync(profileName, solutionUniqueName, CancellationToken.None).ConfigureAwait(false);
        return solution.Id;
    }
}
