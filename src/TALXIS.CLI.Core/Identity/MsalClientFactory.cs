...
/// correct Entra authority. Callers (connection providers, auth commands)
/// never instantiate MSAL builders directly — this is the single place
/// that pins our client constants and authority policy.
/// </summary>
/// <remarks>
...
/// </remarks>
public sealed class MsalClientFactory
{
    /// <summary>
    ...
    /// <summary>
    /// Internal authority resolution: uses explicit cloud on Connection /
    /// Credential, falling back to <see cref="CloudInstance.Public"/>.
    /// Callers that need URL-based cloud inference should resolve the cloud
    /// before passing the connection (e.g. via <c>DataverseCloudMap.TryInferFromEnvironmentUrl</c>).
    /// </summary>
    internal static Uri ResolveAuthority(Connection connection, Credential? credential = null)
    {
        var cloud = connection.Cloud ?? credential?.Cloud ?? CloudInstance.Public;
        var tenant = connection.TenantId ?? credential?.TenantId;
        return EntraCloudMap.BuildAuthorityUri(cloud, tenant);
    }
}
