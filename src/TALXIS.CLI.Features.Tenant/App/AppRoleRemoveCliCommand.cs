...
/// Usage: <c>txc tenant app role remove --app &lt;client-id-or-object-id&gt; --role &lt;name-or-guid&gt; --yes</c>
/// </summary>
[CliDestructive("Removes the tenant-wide role assignment. If --role admin-application is used, this also revokes the app's ability to call txc environment admin commands non-interactively.")]
[CliCommand(
    Name = "remove",
...
public class AppRoleRemoveCliCommand : ProfiledCliCommand, IDestructiveCommand
{
    ...
    [CliOption(Name = "--role", Description = "Tenant role name or GUID. Use 'admin-application' to revoke this app's ability to call txc environment admin commands non-interactively.", Required = true)]
    ...
    private async Task<int> ExecuteRemoveRoleAsync()
    {
        try
        {
            await TenantAppCommandSupport.RemoveAssignmentAsync(Profile, App, Role, CancellationToken.None).ConfigureAwait(false);
...
#pragma warning disable TXC003
                OutputWriter.WriteLine($"Role '{Role}' removed from app '{App}'.");
