...
txc env setting set <setting> <value>
```
...
```sh
txc env setting set appcopilotenabled 0
```
...
```sh
txc env setting set paipreviewscenarioenabled false
```

**Disable AI Prompts:**
```sh
txc env setting set aipromptsenabled false
```

**Disable the maker Copilot bot:**
```sh
txc env setting set powerappsmakerbotenabled false
```
...
```sh
txc env setting set PowerApps_AllowCodeApps true
```
...
```sh
txc env setting set isauditenabled true
```
...
```sh
txc env setting set maxuploadfilesize 131072
```
...
```sh
txc env setting set blockedattachments "exe;bat;com;cmd"
```
